...
# ── Page config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="TEHQeeq AI · AI Research Agent",
    page_icon="🔬",
    layout="wide",
...
.stApp {
    background: #0F1226;
    background-image:
        radial-gradient(ellipse 80% 50% at 20% -10%, rgba(108,92,231,0.12) 0%, transparent 60%),
        radial-gradient(ellipse 60% 40% at 80% 110%, rgba(0,206,201,0.08) 0%, transparent 55%);
}
...
.step-card.active::before { background: #6C5CE7; }
.step-card.done::before   { background: #00CEC9; }
...
/* ── Progress text ── */
.stSpinner > div { color: #6C5CE7 !important; }
...
st.markdown("""
<div class="notice">
    TEHQeeq AI · Powered by LangChain multi-agent pipeline · Built with Streamlit
</div>
""", unsafe_allow_html=True)